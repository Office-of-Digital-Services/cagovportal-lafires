...
export class CaGovLAFiresMap extends window.HTMLElement {
  ...
  openPopup(item, marker) {
    ...
    if (this.open_marker != undefined) {
      // close any open marker
      this.open_marker.setIcon(this.regIcon);
    }
    ...
    document
      .querySelector(`#provider-address-div`)
    ...
    if (window.innerWidth > 768) {
      if (item.lng > cur_center.lng) {
        ...
        const new_center = L.latLng(
          cur_center.lat,
          cur_center.lng + (cur_bounds.getEast() - cur_bounds.getWest()) / 4
        );
...
window.customElements.define("cagov-lafires-map", CaGovLAFiresMap);
